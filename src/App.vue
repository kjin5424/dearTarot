  ...
  <div id="game-container">
    ...
    <div id="vue-overlay">
      <DialogBox
        ...
        speaker="소녀"
        @done="store.goTo('QUESTION_INPUT')"
      />
      ...
      <SpreadAsk
        v-if="scene === 'SPREAD_QUIZ'"
        @selected="onSpreadSelected"
      />

      <KarmaOptions
        v-if="scene === 'KARMA'"
        @selected="onKarmaSelected"
      />
...
const BLOCKED_KEYWORDS = ['죽', '자살', '살인', '재해', '질병', '사망', '암', '전쟁']
...
function onQuestionSubmit(text: string) {
  ...
  if (blocked) {
    ethicsMessage.value = '이 숲의 카드는 그 질문에 답하기 어려워요. 당신을 외면하는 게 아니에요.'
    ethicsSuggestion.value = '대신 이런 질문은 어떨까요? "지금 내가 할 수 있는 것은 무엇일까?"'
    ethicsVisible.value = true
    return
  }
  store.setQuestion(text)
  store.goTo('SPREAD_QUIZ')
}
